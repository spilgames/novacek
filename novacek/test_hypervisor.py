...
import unittest2
import compute
import hypervisor.ovs as ovs
import hypervisor.libvirt_qemu as libvirt_qemu
import auth
import network
import sys
import socket
...
class TestHypervisor(unittest2.TestCase):
    ...
    def setUpClass(self):
        ...
        hv = compute.get_hypervisor(s, socket.gethostname())
        self.s = s
        self.hypervisor = auth.get_novaclient(s)
        self.neutron = auth.get_neutronclient(s)
        self.vlan_map = ovs.get_vlan_map()
        self.libvirt_instances = libvirt_qemu.get_libvirt()
        self.os_instances = compute.get_instances(s, hv)
    ...
    def test_vm_ghosts(self):
        ...
        hypervisor_uuids = [vm.id for vm in self.os_instances]
        for vm in self.libvirt_instances.keys():
            self.assertIn(vm, hypervisor_uuids)
